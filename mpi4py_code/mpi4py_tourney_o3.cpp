...
void custom_reduce(void *in, void *inout, int *len, MPI_Datatype *dptr) {
	...
	arma::mat y_mat = arma::mat(r_g, c_g); y_mat.zeros();
	double inf = std::numeric_limits<double>::infinity();

	double curr_x_col = -1; double curr_y_col = -1;
	while (i < *len) { 
		if (*ind == inf) {
			ind++; curr_x_col = *ind; ind++;
		} else {
			if (curr_x_col != inf) {
				double curr_x_elem = *ind; ind++;
				double curr_x_row = *ind; ind++; 
				x_mat(curr_x_row, curr_x_col) = curr_x_elem;
			}
		}
		if (*inoutd == inf) {
			inoutd++; curr_y_col = *inoutd; inoutd++;
		} else {
			if (curr_y_col != inf) {
				double curr_y_elem = *inoutd; inoutd++;
				double curr_y_row = *inoutd; inoutd++;
				y_mat(curr_y_row, curr_y_col) = curr_y_elem;
			}
		}
		i += 2;
	}
    ...
    arma::mat temp100 = arma::join_cols(proc_v_B, arma::join_cols(indices_B, real_indices_B)); int c_curr = 0;

    src.submat(0, 0, n_g + 3 - 1, A_B_.n_cols - 1) = arma::join_cols(A_B_, temp100); src(n_g + 3, 0) = A_B_.n_cols; 

    for (int index_c = 0; index_c < A_B_.n_cols; index_c++) {
    	*inout_ret = inf; inout_ret++; *inout_ret = index_c; inout_ret++;
    	c_curr += 2;
    	for (int index_r = 0; index_r <= (n_g + 3); index_r++) {
    		if (src(index_r, index_c) != 0) {
    			if (c_curr >= *len) {
    				std::cout << "yo help me out my dudemon \n";
    			} else {
	    			*inout_ret = src(index_r, index_c); inout_ret++; *inout_ret = index_r; inout_ret++;
	    			c_curr += 2;
	    		}
    		}
    	} 
    ...
    if (c_curr > *len) {
		std::cout << "yo help me out my dudette \n";
	}
    src.submat(0, block, n_g - 1, block + 1 - 1) = b_sol__;
    *inout_ret = inf; inout_ret++; *inout_ret = block; inout_ret++; c_curr += 2;
    for (int index_r = 0; index_r < n_g; index_r++) {
    	if (src(index_r, block) != 0) {
    		if (c_curr >= *len) {
    			std::cout << "yo help me out my dudester \n";
    		} else {
				*inout_ret = src(index_r, block); inout_ret++; *inout_ret = index_r; inout_ret++;
				c_curr += 2;
			}
    	}
	}
	...
	if (condition) {
	    ...
	    *inout_ret = inf; inout_ret++; *inout_ret = block + 1; inout_ret++; c_curr += 2;
	    for (int index_r = 0; index_r < l_L_append_1; index_r++) {
	    	if (src(index_r, block + 1) != 0) {
	    		if (c_curr >= *len) {
	    			std::cout << "yo help me out my dudester \n";
	    		} else {
					*inout_ret = src(index_r, block + 1); inout_ret++; *inout_ret = index_r; inout_ret++;
					c_curr += 2;
				}
	    	}
		}
	    src.submat(0, block + 2, l_L_append_2 - 1, block + 3 - 1) = arma::reshape(L_append_2.t(), l_L_append_2, 1);
	    *inout_ret = inf; inout_ret++; *inout_ret = block + 2; inout_ret++; c_curr += 2;
	    for (int index_r = 0; index_r < l_L_append_2; index_r++) {
	    	if (src(index_r, block + 2) != 0) {
	    		if (c_curr >= *len) {
    				buffer.push_back(src(index_r, block + 2)); buffer.push_back(index_r);
    			} else {
					*inout_ret = src(index_r, block + 2); inout_ret++; *inout_ret = index_r; inout_ret++; 
					c_curr += 2;
				}
	    	}
		}
...
stdvec mpi4py_tournament_bLARS_svm_clever(double block) {
    ...
    for (int counter = 0; counter < iterations; counter++) {
        ...
        // stdvec src_vector = mat2vec(src); 
        arma::sp_mat src_sparse = arma::sp_mat(src); stdvec src_vector; double current_col = -1;
        arma::sp_mat::iterator it = src_sparse.begin(); arma::sp_mat::iterator it_end = src_sparse.end();
        double inf = std::numeric_limits<double>::infinity();

        for(; it != it_end; ++it) {
        	if (current_col != it.col()) {
        		src_vector.push_back(inf); current_col = it.col(); src_vector.push_back(it.col());
        	}
        	src_vector.push_back(*it); src_vector.push_back(it.row());  
		}
		...
		while (src_vector.size() < (req_size[0])) {
			src_vector.push_back(inf);
		}

        double src_arr[req_size[0]]; double dst_arr[req_size[0]];
        for (int i = 0; i < src_vector.size(); i++) {src_arr[i] = src_vector[i];}

        MPI_Allreduce(src_arr, dst_arr, req_size[0], MPI_DOUBLE, op, MPI_COMM_WORLD); int ct_temp = 0;
