...
void custom_reduce(void *in, void *inout, int *len, MPI_Datatype *dptr) {
  ...
  //       	src_vector.push_back(*it); src_vector.push_back(it.row()); src_vector.push_back(it.col()); 
    ...
    arma::vec b_sol__ = d[0]; arma::vec B_ = d[1]; arma::mat A_B_ = d[2]; arma::mat L_append_1; arma::mat L_append_2;

    if (condition) {
    	L_append_1 = d[3]; L_append_2 = d[4]; 
    }
    stdvec B_converted = arma::conv_to<stdvec>::from(B_);

    proc_v.reshape(1, proc_v.n_elem); arma::mat proc_v_B = selectCols(proc_v, B_converted);
    indices.reshape(1, indices.n_elem); arma::mat indices_B = selectCols(indices, B_converted);
    real_indices.reshape(1, real_indices.n_elem); arma::mat real_indices_B = selectCols(real_indices, B_converted);
    ...
    for (int index_c = 0; index_c < A_B_.n_cols; index_c++) {
    	for (int index_r = 0; index_r < (n_g + 3); index_r++) {
    		*inout_ret = src(index_r, index_c); inout_ret++;
    ...
    for (int index_r = 0; index_r < n_g; index_r++) {
		*inout_ret = src(index_r, block); inout_ret++;
	...
	if (condition) {
	    ...
	    for (int index_r = 0; index_r < l_L_append_1; index_r++) {
    		*inout_ret = src(index_r, block + 1); inout_ret++;
	    ...
	    for (int index_r = 0; index_r < l_L_append_2; index_r++) {
    		*inout_ret = src(index_r, block + 2); inout_ret++;
...
stdvec mpi4py_tournament_bLARS_svm_clever(double block) {
    ...
    for (int counter = 0; counter < iterations; counter++) {
		...
		std::cout << src_vector.size() << "\n";

		double src_arr[src_vector.size()]; double dst_arr[src_vector.size()];
        ...
        for (int i = 0; i < src_vector.size(); i+=3) {
